# PERSONNAGES PRÉ-TIRÉS — Niveau 1

> **5 personnages prêts à jouer.** Chacun représente une classe différente avec un build optimisé pour le niveau 1. Imprime la fiche, prends tes dés, et joue.

---
---

## 1. CPL. RHEA CALDER — Grounder (Précision)

*Ancienne tireuse d'élite de la milice de HopeTown. Posée, méthodique, ne gaspille pas ses balles. Elle a quitté le service après un incident qu'elle refuse d'évoquer. Aujourd'hui elle travaille en freelance — protection de convois, sécurité privée, tout ce qui paie et qui ne pose pas trop de questions.*

| STR | DEX | WIT | EMP |
|---|---|---|---|
| **10** (+2) | **10** (+2) | 8 (0) | 7 (-1) |

*Array Standard (10, 10, 8, 7)*

| Donnée | Valeur | Calcul |
|---|---|---|
| **PV** | **24** | Vig 10 + bonus STR 2 |
 Vig 10 + bonus STR 2 |
| **Blessures** | 0 / 6 |
...
**Compétences :** Perception (WIT 8 + 1 = **9**), Survie (WIT 8 + 1 = **9**)

**Armes :**

| Arme | Score | Dégâts | Portée | Propriétés |
|---|---|---|---|---|
| Fusil semi-auto | DEX 10 + 1 = **11** (55%) | 1d8 +1d6 | 30 | — |
| Revolver | DEX 10 + 1 = **11** (55%) | 1d6 +1d6 | 8 | — |

> *+1d6 = bonus de dégâts (DEX 10).*
...
**En jeu :** Rhea est le pilier du groupe. Chaque tir compte — 1d8 +1d6 +2 = moyenne 10.5, c'est le meilleur DPR du groupe au niveau 1. Son gilet pare-balles est discret (portée sous une veste civile). 12 PV et 9 surges = elle tient longtemps. Son point faible : WIT 8 et EMP 7 (mauvaise perception, mauvais social).
...
## 2. NAIA SERRANO — Infiltrator

*Ancienne agent de renseignement corpo chez Ryujin Industries. Spécialiste des entrées non autorisées, des extractions silencieuses, et des disparitions. Elle a brûlé son ancienne identité quand son employeur a décidé qu'elle en savait trop. Aujourd'hui elle vit sous un faux nom dans les bas-fonds de Neon, acceptant des contrats d'extraction et de récupération de données.*

| STR | DEX | WIT | EMP |
|---|---|---|---|
| 7 (-1) | **11** (+3) | 9 (+1) | 7 (-1) |

*Array Min-Max (11, 9, 7, 7)*

| Donnée | Valeur | Calcul |
|---|---|---|
| **PV** | **14** | Vig 8 + bonus STR -1 |
 Vig 8 + bonus STR -1 |
| **Blessures** | 0 / 6 |
...
**Compétences :** Discrétion (DEX 11 + 1 + 2 expertise = **14**, 70%), Acrobatie (DEX 11 + 1 = **12**), Perception (WIT 9 + 1 = **10**), Escamotage★ (DEX 11 + 1 = **12**)

**Armes :**

| Arme | Score | Dégâts | Portée | Propriétés |
|---|---|---|---|---|
| Pistolet semi-auto | DEX 11 + 1 = **12** (60%) | 1d6 +1d6 | 10 | — |
| Lame de combat | DEX 11 + 1 = **12** (60%) | 1d6 +1d6 | 1 | Finesse |

> *+1d6 = bonus de dégâts (DEX 11). La lame utilise DEX grâce à Finesse.*
...
- **Expertise Furtive** — +2 permanent aux jets de Discrétion (déjà inclus dans le Score 14).

**En jeu :** Naia est un scalpel. Fragile (7 PV, 4 surges) mais mortelle quand les conditions sont réunies. Sa boucle : se cacher (Discrétion 14 = 70%), frapper avec Frappe Ciblée (14 dégâts moyens), se replier. Elle meurt vite si elle se fait repérer — le Grounder et le Marshal doivent la protéger. Sa Discrétion à 14 est le Score le plus élevé du groupe.
...
Lame de combat      Score 12 (60%)  1d6+1d6       Reach 1 (Finesse)
  + Frappe Ciblée: +1d6 si conditions remplies
...
## 3. DR. TOMÁS ACHEBE — Medic

*Chirurgien urgentiste qui a tout lâché après deux ans dans un hôpital de guerre sur la frontière coloniale. Il a vu trop de soldats mourir sur sa table faute de matériel. Aujourd'hui il travaille avec des équipes indépendantes — mercenaires, explorateurs, contrebandiers. Il soigne tout le monde sans poser de questions. Ses mains ne tremblent jamais, même sous le feu.*

| STR | DEX | WIT | EMP |
|---|---|---|---|
| 9 (+1) | 8 (0) | **10** (+2) | 9 (+1) |

*Array Balanced (10, 9, 9, 8)*

| Donnée | Valeur | Calcul |
|---|---|---|
| **PV** | **18** | Vig 8 + bonus STR 1 |
 Vig 8 + bonus STR 1 |
| **Blessures** | 0 / 6 |
| **Armure** | **3** | Gilet pare-balles souple |
| **Speed** | 6 cases | |
...
**Compétences :** Médecine★ (WIT 10 + 1×2 doublé = **12**, 60%), Sciences★ (WIT 10 + 1 = **11**), Perception (WIT 10 + 1 = **11**), Persuasion (EMP 9 + 1 = **10**)

**Armes :**

| Arme | Score | Dégâts | Portée | Propriétés |
|---|---|---|---|---|
| Pistolet semi-auto | DEX 8 + 1 = **9** (45%) | 1d6 +1d4 | 10 | — |

> *+1d4 = bonus de dégâts (DEX 8).*
...
## 4. LT. JUNO OKAFOR — Marshal

*Officier de la Freestar Collective, formée à l'académie d'Akila City. Spécialiste en commandement de petites unités. Elle ne crie jamais — elle n'en a pas besoin. Quand Juno parle, les gens écoutent. Pas parce qu'elle est effrayante, mais parce que ses ordres ont toujours raison. Elle a été déchargée honorablement après avoir refusé un ordre qu'elle jugeait immoral. Aujourd'hui elle monte sa propre équipe.*
...
**Compétences :** Tactique★ (WIT 7 + 1 = **8**, 40%), Intimidation (EMP 10 + 1 = **11**), Persuasion (EMP 10 + 1 = **11**), Intuition (EMP 10 + 1 = **11**)

**Armes :**

| Arme | Score | Dégâts | Portée | Propriétés |
|---|---|---|---|---|
| Fusil semi-auto | DEX 8 + 1 = **9** (45%) | 1d8 +1d4 | 30 | — |
| Revolver | DEX 8 + 1 = **9** (45%) | 1d6 +1d4 | 8 | — |

> *+1d4 = bonus de dégâts (DEX 8). Juno ne tire pas bien, mais au moins elle contribue.*
...
Ordres: "Concentre ton tir!" (Avantage +2 dégâts)
...
## 5. SUKI "PATCH" TANAKA — Tech

*Bidouilleuse née. Suki a grandi dans les coursives de Deimos Staryard, entourée de moteurs, de câbles et de robots de maintenance. À 14 ans elle avait déjà construit son premier drone. À 18, elle s'est fait virer du programme d'ingénierie de Nova Galactic pour avoir "emprunté" des composants. Aujourd'hui elle fait voler son drone, elle pirate ce qui a besoin d'être piraté, et elle répare tout ce qui tombe en panne. Son surnom vient du nombre de patches qu'elle a collés sur son drone — il ne ressemble plus à rien, mais il vole mieux que neuf.*
...
**Compétences :** Ingénierie★ (WIT 11 + 1×2 doublé = **13**, 65%), Informatique★ (WIT 11 + 1 = **12**, 60%), Perception (WIT 11 + 1 = **12**), Sciences★ (WIT 11 + 1 = **12**)
...
**Drone éclaireur "Rustbucket" :**

| | |
|---|---|
| **PV** | **6** (3 + bonus WIT × niveau = 3 + 3×1) |
| **Blessures** | 0 / 6 |
| **Armure** | 0 |
| **Vitesse** | Vol 8 cases |
| **Portée contrôle** | 100 cases |
| **Capteurs** | Caméra vidéo/audio |

*Commandes (1 action chacune) :* Déplacer, Observer (tu vois ce que le drone voit), Marquer (Avantage pour les alliés contre la cible), Revenir.
...
- **Compétences Techniques** — Proficiency doublée en Ingénierie (déjà incluse). Réparations 2× plus rapides.

**En jeu :** Suki est le personnage le plus fragile du jeu. 5 PV, Armure 2, 3 surges. Un seul tir de fusil peut la mettre à terre. Sa force : le drone. Marquer une cible (Avantage pour tout le groupe) est extrêmement puissant — c'est comme si chaque allié avait Visé gratuitement. Son Ingénierie à 13 (65%) est le meilleur Score technique du groupe. Elle répare le loot, crochète les systèmes, et pirate les terminaux. **Elle ne doit jamais être en première ligne.**
...
| | Rhea (Grounder) | Naia (Infiltrator) | Tomás (Medic) | Juno (Marshal) | Suki (Tech) |
|---|---|---|---|---|---|
| **PV** | 12 | 7 | 9 | 10 | 5 |
| **Blessures** | 0 / 6 |
| **Armure** | 3 | 4 | 3 | 5 | 2 |
| **Surges** | 9 | 4 | 7 | 8 | 3 |
| **Score attaque** | 11 (55%) | 12 (60%) | 9 (45%) | 9 (45%) | 10 (50%) |
| **Dégâts/tir** | 1d8+1d6+2 (10.5) | 1d6+1d6 (7) | 1d6+1d4 (6) | 1d8+1d4 (7) | 1d4+1d4 (5) |
| **Burst (conditions)** | — | +1d6 (14) | — | — | — |
| **Meilleur Score** | Save STR/DEX 11 | Discrétion 14 | Médecine 12 | Persuasion 11 | Ingénierie 13 |
| **Point faible** | WIT/EMP | PV/Surges | Attaque | Attaque | Tout sauf WIT |
| **Rôle** | DPR, front-line | Burst, furtivité | Soins, surges | Ordres, boost | Drone, hack, repair |

---

# CONSEILS POUR LES JOUEURS

**Rhea (Grounder) :** Tu es le pilier. Reste en mouvement (Cadre Solide = pas de Désavantage), tire avec méthode. Chaque tir fait 10.5 dégâts en moyenne — c'est le meilleur DPR du groupe. Protège les alliés fragiles.
...
# L'ESCOUADE EN ACTION

**Tour typique (combat contre 4 mercenaires) :**
...
**Résultat :** 24+ dégâts concentrés sur une cible en un round. Un mercenaire standard (PV 20-30, Armure 4-5) tombe en 2 rounds. L'escouade fonctionne parce que chaque rôle est complémentaire.
